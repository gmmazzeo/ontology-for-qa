...
public class DBpediaOntology implements Ontology {
    ...
    final static String classesUrl = "http://mappings.dbpedia.org/server/ontology/classes/";
    final static String classesBaseUri = "http://dbpedia.org/ontology/";
    final static String sparqlEndpoint = "http://dbpedia.org/sparql";
    final static String superpagesFile = "/home/massimo/superpages.txt";
...
/*
    private void setNameThumbAndUrl(DBpediaNamedEntity ne) {
        ...
        try {
            ...
            System.out.println(sparqlEndpoint + " is DOWN");
        } finally {
            qexec.close();
...
*/
        ...
        for (Element c : e.children()) {
            ...
            if (tagName.equals("a")) {
                ...
                if (href != null && href.length() > 0) {
                    ...
                    category.setUri(classesBaseUri + href);
                    map.put(category.getLabel(), category);
                    System.out.println(c.text() + "\t" + classesBaseUri + href);
                }
    ...
    private DBpediaOntology() {
        ...
        Document doc = null;
        try {
            doc = Jsoup.connect(classesUrl).get();
        } catch (IOException ex) {
            Logger.getLogger(DBpediaOntology.class.getName()).log(Level.SEVERE, null, ex);
        }

        traverseHierarchy(doc.body().children().get(1).children().get(0), root, categoryMap);

        File dir = new File("/home/massimo/DBpedia csv");
        ...
        try {
            typicalityEvaluator = new TypicalityEvaluator("/home/massimo/DBpedia csv/counts.bin");
        } catch (IOException ex) {
            Logger.getLogger(DBpediaOntology.class.getName()).log(Level.SEVERE, null, ex);
        ...
        try {
            BufferedReader in = new BufferedReader(new FileReader(superpagesFile));
